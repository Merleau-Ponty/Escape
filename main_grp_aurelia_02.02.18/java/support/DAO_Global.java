...
import com.aureliasarradin.escapegamea.Avatar;
...
public class DAO_Global {
    ...
    public ArrayList<Equipe> donneLesEquipes(){
        ArrayList<Equipe> liste = new ArrayList<Equipe>();
        Cursor c = maBase.rawQuery("SELECT eq_id, eq_nom, eq_avatar FROM equipe;", null);
        while (c.moveToNext()){
            int id = c.getInt(0);
            String nom = c.getString(1);
            String avatar = c.getString(2);
            Equipe equipe = new Equipe(id,nom,avatar);
            liste.add(equipe);
        }
        return liste;
    }

    public int donneLeScore(Equipe equipe){
        int score = 0;
        Cursor c = maBase.rawQuery("select p_score from partie where eq_id = "+equipe.getEq_id()+";", null);
        while (c.moveToNext()){
            score = c.getInt(0);
        }
        return score;
